  ...
  useEffect(() => {
    ...
    function displayMarker(y, x, mId, dName, rpMin, rpMax, pMin, pMax, aMin, aMax, lLon, rLon, tLat, bLat) {
      ...
      kakao.maps.event.addListener(marker, 'click', function () {
        ...
        const clustersData_url = `https://k7c208.p.ssafy.io:8080/api/v1/estate/articles?markerId=${mId}&page=1&dongName=${dName}&rentPriceMin=${rpMin}&rentPriceMax=${rpMax}&priceMin=${pMin}&priceMax=${pMax}&areaMin=${aMin}&areaMax=${aMax}&leftLon=${lLon}&rightLon=${rLon}&topLat=${tLat}&bottomLat=${bLat}`
        axios
    ...
    kakao.maps.event.addListener(map, 'center_changed', function () {
      ...
      async function getDongName() {
        try {
          ...
          let clusters_url = `https://k7c208.p.ssafy.io:8080/api/v1/estate/clusters?dongName=${region_dong}&rentPriceMin=${monthlyMin}&rentPriceMax=${monthlyMax}&priceMin=${saleAndDepositMin}&priceMax=${saleAndDepositMax}&areaMin=${roomSizeMin}&areaMax=${roomSizeMax}&leftLon=${leftLon}&rightLon=${rightLon}&topLat=${topLat}&bottomLat=${bottomLat}`;
