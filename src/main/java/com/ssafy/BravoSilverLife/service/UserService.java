...
//        System.out.println("@@@"+findUser.getId());
    ...
    public int changePhoneNumber(String id, String authNumber, String newPhoneNumber) {
        User user = userRepository.findById(id); //기존번호로 user 정보 찾고
        ...
        if ((phoneAuth.getPhoneAuth()).equals(authNumber)) { // 새 폰번호 인증번호 확인 시
            user.changePhoneNumber(newPhoneNumber); // user 정보에서 핸드폰 번호 변경
        }
